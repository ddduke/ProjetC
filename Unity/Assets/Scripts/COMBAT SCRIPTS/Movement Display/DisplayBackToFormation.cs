...
public class DisplayBackToFormation : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //UpdateFormationInfo("enemy"); ==> No need of this line since there is no regiment at the start for the moment, maybe later when enemy regs will be automatically positionned ?
        //UpdateFormationInfo("player");==> No need of this line since there is no regiment at the start for the moment, maybe later when enemy regs will be automatically positionned ?
    ...
    public void LaunchScriptWithChargeInFormation()
    {
        side = GetComponent<TurnManager>().turn;
        GetComponent<OrderManagement>().actualOrder = "BackInFormation";
        BackToFormationInCharge();
    }
    ...
    public void BackToFormationInCharge()
    {
        
        target = target = GetComponent<UsefulCombatFunctions>().FormationGetNearestEnemyPositionToCharge(side);
        if (side == "player")
        {
            target.z = GetComponent<UsefulCombatFunctions>().CorrectTargetZWithMinMax(target.z, playerMinZFormation, playerMaxZFormation);
            target.x = GetComponent<UsefulCombatFunctions>().CorrectTargetXWithMinMax(target.x, playerMinXFormation, playerMaxXFormation);
