...
public class OrderManagement : MonoBehaviour
{
    ...
    public void launchNewOrder(int order)
    {
        ...
        if (order == 0) //Order : Move in formation
        {
            ...
            {
                reg.GetComponent<CombatVariables>().inFormationOrder = true;
                reg.GetComponent<SpeedCalulator>().CheckSpeed();
            }
        }
        ...
        if(order ==1) //Order : Charge In Formation
        {
            if (!GetComponent<UsefulCombatFunctions>().sideInFormation(side))
            {
                GetComponent<DisplayBackToFormation>().LaunchScriptWithChargeInFormation();
        ...
        if (order == 2) //Order : Charge and Break Formation 
        {
            ...
            foreach (GameObject reg in regimentsList)
            {
                reg.GetComponent<CombatVariables>().inFormationOrder = false;
                reg.GetComponent<SpeedCalulator>().CheckSpeed();
            }
        }
